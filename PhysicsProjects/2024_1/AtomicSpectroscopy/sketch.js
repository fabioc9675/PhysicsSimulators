...
img = loadImage( "images/panel.png");
img2 = loadImage("images/TuboDescarga.png");
}
...
longitudesOnda = [379,438,443,447,471,492,501,504,586,587,588,667,781];
rangosEmision = [];
break;
case "Nitrógeno diatómico":
luzemitida=[224, 168, 94]
longitudesOnda = [379,409,433,485,584,593,608,613,625,632,639,650,655,667,671,702,776,781];
rangosEmision = [];
break;
case "Neón":
luzemitida=[256, 80, 40]
longitudesOnda = [379,540,585,588,603,607,616,621,626,633,638,640,650,659,692,703,781];
rangosEmision = [];
break;
case "Sodio":
...
longitudesOnda = [379,425, 430,435,554,555,556,585,586,587,758,759,760,761,768,781];
rangosEmision = [];
break;
case "Yodo":
luzemitida=[256, 120, 120]
longitudesOnda = [379,403,404,434,435,436,544,545,546,547,576,577,695,696,705,706,714,728,737,738,749,750,751,762,763,771,772,781];
rangosEmision = [];
break;
case "Mercurio":
luzemitida=[200,218,230]
longitudesOnda = [379,403,404,405, 407,432,434,436,437,438,491,544,545,546,547,548,575,576,577,579,580,689,690,691,781];
rangosEmision = [];
break;
case "Ácido Sulfhídrico":
luzemitida = [140, 240, 256];
longitudesOnda = [379,380,387,392,397,404,412,418,425,433,439,447,455,462,470,479,485,491,516,577,587,655,781];
rangosEmision = [];
break;
case "Compuesto desconocido":
luzemitida = [200,200,200];
longitudesOnda = [379,403,404,434,435,436,544,545,546,547,568,576,577,585,586,587,590,591,592,695,696,705,706,714,728,737,738,749,750,751,762,763,771,772,781];
rangosEmision = [];
break;
}
for (let i = 0; i < longitudesOnda.length - 1; i++) {
rangosEmision.push({
inicio: longitudesOnda[i] + 1, fin: longitudesOnda[i+1] - 1},
)}
}
...
drawSpectrum(x, (canvasHeight - spectrumHeight) / 2,
spectrumWidth, spectrumHeight, rangosEmision);
...
// Dibuja la imagen
image(img, imageX+75, (canvasHeight - imageHeight) / 2, imageWidth, imageHeight);
...
drawWave(element, numPoints, amplitudOnda, longitudOndaMin, longitudOndaMax, canvasWidth, canvasHeight, scaleHeight, rangosEmision);
drawWhiteLight(element, numPoints, amplitudOnda, canvasWidth, canvasHeight)
}
...
let y =
lerp(startY, endY, t) +
canvasWidth * sin((25 * PI * t)); // Coordenada y ondulada
curveVertex(x, y);
}
